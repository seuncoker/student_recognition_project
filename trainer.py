...
class Trainer(object):
    ...
    def __init__(self, cmd, cuda, model, criterion, optimizer,
                 ...
                 checkpoint_dir=None, print_freq=1, meta_file=None):
        """
        :param cuda:
        :param model:
        :param optimizer:
    ...
    def validate(self):
        ...
        class_name_dict = utils.get_class_name_map(self.meta_file)
        ...
        for batch_idx, (imgs, target, img_files, class_ids) in tqdm.tqdm(
            ...
            print("output: ", torch.argmax(output.data, dim=-1))
            print("target: ", target.data)
            # print(class_name_dict.keys())
            # print(class_name_dict.values())
            # print("Prediction: ", [class_name_dict[int(i)] for i in torch.argmax(output, dim=-1)])
            # print("True: ", [class_name_dict[int(i)] for i in target])
...
class Validator(Trainer):
    ...
    def __init__(self, cmd, cuda, model, criterion, val_loader, log_file, meta_file, print_freq=1):
        super(Validator, self).__init__(cmd, cuda=cuda, model=model, criterion=criterion,
                                        val_loader=val_loader, log_file=log_file, print_freq=print_freq, meta_file=meta_file,
