...
def get_id_label_map(meta_file):
    N_IDENTITY = 10 #21 #9131  # total number of identities in VGG Face2
    N_IDENTITY_PRETRAIN = 10 #21 #8631  # the number of identities used in training by Caffe
    identity_list = meta_file
    df = pd.read_csv(identity_list, sep=',\s+', quoting=csv.QUOTE_ALL, encoding="utf-8", engine='python')
    print("df: ", df)
    df["class"] = -1
...
def get_class_name_map(meta_file):
    N_IDENTITY = 10 #21 #9131  # total number of identities in VGG Face2
    N_IDENTITY_PRETRAIN = 10 #21 #8631  # the number of identities used in training by Caffe
    identity_list = meta_file
    df = pd.read_csv(identity_list, sep=',\s+', quoting=csv.QUOTE_ALL, encoding="utf-8", engine='python')
    ...
    key = df["class"].values
    val = df["Name"].values
    id_name_dict = dict(zip(key, val))
    #print("id_label_dict: ", id_name_dict)
    return id_name_dict
