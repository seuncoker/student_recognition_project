...
import datasets.VGG_Faces2.vgg_face2 as datasets
...
configurations = {
    1: dict(
        ...
        momentum=0.9,
        weight_decay=0.0,
        gamma=0.5, # "lr_policy: step"
        step_size=25, # "lr_policy: step"
...
def main():
    ...
    N_IDENTITY = 10  # the number of identities in VGGFace2 for which ResNet and SENet are trained
    N_IDENTITY_NEW = args.num_classes  # the number of identities in VGGFace2 for which ResNet and SENet are trained
    ...
    if 'resnet' in args.arch_type:
        if args.cmd == 'train' and not resume:
            model = ResNet.resnet50(num_classes=N_IDENTITY, include_top=include_top)
        else:
            model = ResNet.resnet50(num_classes=N_IDENTITY_NEW, include_top=include_top)
    else:
    ...
    if args.cmd == 'train':
        ...
        if N_IDENTITY != N_IDENTITY_NEW:
            in_features = model.fc.in_features
            model.fc = nn.Linear(in_features, N_IDENTITY_NEW) # Replace the fully connected layer
    ...
    if args.cmd == 'train':
        ...
        optim = torch.optim.Adam(classifier_params, lr=cfg['lr'])
